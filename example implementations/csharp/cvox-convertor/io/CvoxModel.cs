...
namespace cvox_convertor.io
{
    public class CvoxModel : List<Cube>
    {
            ...
            for (int xx = 0; xx < size.X; xx++)
                for (int yy = 0; yy < size.Y; yy++)
                    ...
                    { // for every possible voxel location
                        ...
                        if (i != 0)
                        {
                            XYZ low = new(xx, yy, zz); // lowest point of the cube
                            XYZ high = new(xx, yy, zz); // highest point of the cube
        ...
        private static void ExpandSideIfTrue(bool check, int side, XYZ high)
        {
            if (check)
